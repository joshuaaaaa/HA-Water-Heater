...
### Fixed
- **Critical:** Fixed card registration issue where card was not found by Home Assistant
  - Changed customCards registration from `type: 'ha-boiler-card'` to `type: 'custom:ha-boiler-card'`
  - This fixes the "custom element not found" error that occurred after installation
  - Card now properly appears in Home Assistant's card picker
